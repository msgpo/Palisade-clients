...
public class SimpleClient<T> {
     ...
     * @throws IOException if deserialisation of the data-service response failed
     */
    public Stream<Stream<T>> readResponse(final String token, final Set<LeafResource> resourceSet) {
     ...
     * The top-level stream returns open connections to the data-service to read a single LeafResource.
     * The sub-streams return records one-by-one from a resource.
     * Be wary of how these streams are evaluated and used, as a collect(toList()) will open up multiple data-service
     * connections that may time-out if resources are not actually read off the stream.
     *
     * @param response the {@link DataRequestResponse} from a previous call to the palisade-service
     * @return a stream of objects of type T from the data-service
     * @throws IOException if deserialisation of the data-service response failed
     */
    public Stream<Stream<T>> readResponse(final DataRequestResponse response) {
